...
function selectDph(){
    ...
    console.log(dphPacks[dphSelect.selectedIndex - 1].ID)
}
...
async function postFunction(){
    ...
    if(authedDph){
        ...
        formData.append("minecraft_versions", mcVersions.toString());
        ...
        fetch(`https://api.datapackhub.net/versions/new/${selectedDphPack}`, {
            method: "POST",
            headers: {
                Authorization: "Basic " + dphToken,
            },
...
<div class="flex justify-around items-center min-h-screen h-full py-3" on:dragover|preventDefault="{() => {}}" on:drop|preventDefault="{event => drop(event)}">
    <div class="flex flex-col items-center space-y-10">
        ...
        <div>
            <div class="bg-zinc-950 rounded-xl border-zinc-800 border-2 p-3 max-h-[50%] overflow-y-auto styled-scrollbar max-w-screen-lg" use:autoAnimate bind:this={body}>
                ...
                <div class="flex flex-col md:flex-row justify-around space-y-2 md:space-y-0 md:space-x-2 w-full">
                    ...
                    <div class="bg-zinc-950 w-full rounded-xl border-zinc-800 border-2 p-3 space-y-2">
                        ...
                        bind:value={modToken} />
                        <button class="items-center flex w-fit bg-zinc-900 rounded-md h-8 p-2 px-2 cursor-pointer text-zinc-400 hover:text-zinc-300 mb-1" on:click={authModrinth}>Authenticate</button>
                        {:else}
                ...
                <div class="flex">
                    <div class="w-1/3">
                        ...
                        {#if addModDep == false}
                        <button class="bg-zinc-900 rounded-md h-8 mb-2 p-1 px-2 text-zinc-400 hover:text-zinc-300" on:click={() => addModDep = true}>Add Dependency</button>
                        {#key modDeps}
                            {#each modDeps as dep}
                ...
                <div class="p-3 bg-zinc-900 rounded-xl max-w-full">
                    ...
                    {#if rpfile}
                    <p class="text-zinc-400"><b class="text-zinc-200">Resource Pack: </b> {rpfile.name} (Required)</p>
                    {/if}
            ...
            <div class="mt-1 flex">
                ...
                <div class="w-1/3">
                    <button class="transition-all {authed > 0 ? "bg-orange-600" : "bg-orange-600/40 cursor-default"} rounded-md p-1 px-2 float-right text-lg font-semibold" on:click={() => {
                        if(page == 3) postFunction();
                        if(authed > 0 && page != 3) page++; 
