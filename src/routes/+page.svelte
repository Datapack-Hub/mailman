...
import { IconCheck, IconDeviceFloppy, IconEye, IconFile, IconFileDescription, IconFilter, IconHeading, IconHelp, IconListNumbers, IconLogin2, IconMoodHappy, IconPaperclip, IconPlus, IconTicTac, IconTicket, IconUpload, IconVersions } from '@tabler/icons-svelte';
...
async function authDph(){
    ...
    if(dphUser.ok){
        ...
        let dphProjs = await fetch(`https://api.datapackhub.net/user/${dphUserParsed.username}/projects`)
        if(dphProjs.ok){
...
async function createModDep(){
    ...
    if(modDepSource == "project"){
        ...
        if(projReq.ok){
            let projReqParsed = await projReq.json() as {title: string,id: string,icon_url:string, dep_type: "required" | "optional" | "incompatible" | "embedded"}
...
async function postFunction(){
    if(!dpfile) return alert("Something went wrong. Please upload your project again and try again. If the issue persists, please contact us on our Discord.")
    ...
    if(authedSmithed){
        ...
        let smiUploadData: {name: string, downloads:{datapack?: string, resourcepack?: string}, supports: string[],dependencies?: {id: string, version: string}} = {
            name: versionCode, 
            downloads: {
        ...
        let smiReq = await fetch(`https://api.smithed.dev/v2/packs/${selectedSmiPack}/versions?token=${smithedToken}&version=${versionCode}`, {
            method: "POST",
            headers: {
        ...
        if(smiReq.ok) {smiStatus = "Done!"} else {smiStatus = "Failed."}
    }
}
...
<div class="flex justify-around items-center min-h-screen h-full py-3" on:dragover|preventDefault="{() => {}}" on:drop|preventDefault="{event => drop(event)}">
    <div class="flex flex-col items-center space-y-10">
        ...
        <div>
            <div class="bg-zinc-950 rounded-xl border-zinc-800 border-2 p-3 max-h-[50%] overflow-y-auto styled-scrollbar relative max-w-screen-lg" use:autoAnimate bind:this={body}>
                ...
                <div class="flex flex-col md:flex-row justify-around space-y-2 md:space-y-0 md:space-x-2 w-full">
                    ...
                    <div class="bg-zinc-950 w-full rounded-xl border-zinc-800 border-2 p-3 space-y-2">
                        ...
                        bind:value={modToken} />
                        <Button click={authModrinth}><IconLogin2 /><p>Authenticate</p></Button>
                        {:else}
                ...
                <h2 class="text-sm mt-3 mb-1 flex space-x-1 items-center"><IconHeading /><span>Title</span> <Dots dots={["mod","dph"]} /></h2>
                <input placeholder="The Snails Update" bind:value={title} />

                <h2 class="text-sm mt-3 mb-1 flex space-x-1 items-center"><IconListNumbers /><span>Version Code</span> <Dots dots={["mod","smi","dph"]} /></h2>
                <input placeholder="v1.2.3" bind:value={versionCode} />

                <h2 class="text-sm mt-3 mb-1 flex space-x-1 items-center"><IconFileDescription /><span>Changelog</span> <Dots dots={["mod","dph"]} /></h2>
                <textarea placeholder="This version adds a snail you can worship, among other bug fixes" bind:value={changelog} />

                <h2 class="text-sm mt-3 mb-1 flex space-x-1 items-center"><IconVersions /><span>Supported Minecraft Versions <Dots dots={["mod","dph","smi"]} /></h2>
                <MultiSelect options={minecraftVersions} placeholder="1.17, 1.18" bind:value={mcVersions}/>
                
                <div class="flex">
                    <div class="w-1/3">
                        <h2 class="text-sm mt-3 mb-1 flex space-x-1 items-center"><IconFilter /><span>Modrinth Dependencies</span> <Dots dots={["mod"]} /></h2>
                        {#if addModDep == false}
                        <button class="bg-zinc-900 rounded-md h-8 mb-2 p-1 px-2 text-zinc-400 hover:text-zinc-300 flex items-center space-x-1" on:click={() => addModDep = true}><IconPlus /><span>Add Dependency</span></button>
                        {#key modDeps}
                            {#each modDeps as dep}
                            <div class="flex items-center space-x-2 mb-1">
                                <img src={dep.icon_url} class="h-8 rounded-md" />
                                <h1 class="hover:line-through cursor-pointer" on:click={() => modDeps = modDeps.filter(item => item != dep)}>{dep.title}</h1>
                        ...
                        <div class="space-y-1 w-2/3">
                            ...
                            <input placeholder="{modDepSource == "project" ? "Project ID or slug" : "Version ID"}" bind:value={modDepValue} />
                            ...
                            <div class="flex space-x-1">
                                <button class="bg-orange-600 rounded-md p-1 px-2 text-md" on:click={createModDep}>Add</button>
                                <button class="bg-zinc-800 rounded-md p-1 px-2 text-md" on:click={() => addModDep = false}>Cancel</button>
                            </div>
                        </div>
                        {/if}
                    </div>
                    <div class="w-1/3">
                        <h2 class="text-sm mt-3 mb-1 flex space-x-1 items-center"><IconFilter /><span>Smithed Dependencies</span> <Dots dots={["smi"]} /></h2>
                        {#each [] ?? [] as dep}
                        <h1>{dep}</h1>
                        {/each}
                        {#if addSmiDep == false}
                        <button class="bg-zinc-900 rounded-md h-8 mb-2 p-1 px-2 text-zinc-400 hover:text-zinc-300 flex items-center space-x-1" on:click={() => addSmiDep = true}><IconPlus /><span>Add Dependency</span></button>
                        {#key smiDeps}
                            {#each smiDeps as dep}
                            <div class="flex items-center space-x-2 mb-1">
                ...
                <h2 class="text-sm mt-3 mb-1 flex space-x-1 items-center"><IconDeviceFloppy /><span>Release Channel</span> <Dots dots={["mod"]} /></h2>
                ...
                <h2 class="text-sm mt-3 mb-1 flex space-x-1 items-center"><IconPaperclip /><span>Resource Pack</span> <Dots dots={["mod", "smi", "dph"]} /></h2>

                <input type="file" class="hidden" name="rp" id="rp" bind:this={rpInput} on:change={uploadRP}/>
                <label for="rp" bind:this={rpLabel} class="items-center flex w-fit bg-zinc-900 rounded-md h-8 p-2 px-2 cursor-pointer text-zinc-400 hover:text-zinc-300 mb-1 space-x-1"><IconUpload /><span>Add Resource Pack file</span></label>

                <p class="text-sm text-zinc-500 mt-2 flex space-x-1 items-center"><IconMoodHappy /><span>Nothing entered here is sent to us. All code is client-side, which means that we can't view anything you enter here, including API tokens.</span></p>
                {:else if page == 3}
                <p class="font-bold text-md mb-2 flex items-center space-x-1"><IconEye /><span>Preview your new version:</span></p>
                <div class="p-3 bg-zinc-900 rounded-xl max-w-full">
                    <p class="text-zinc-400 flex items-center space-x-1"><IconHeading class="text-zinc-200" /><b class="text-zinc-200">Title: </b> <span>{title}</span></p>
                    <p class="text-zinc-400 flex items-center space-x-1"><IconListNumbers class="text-zinc-200" /><b class="text-zinc-200">Version Code: </b> <span>{versionCode}</span></p>
                    {#if rpfile}
                    <p class="text-zinc-400 flex items-center space-x-1"><IconFile class="text-zinc-200" /><b class="text-zinc-200">Resource Pack: </b> <span>{rpfile.name} (Required)</p>
                    {/if}
                    <p class="text-zinc-400 flex items-center space-x-1 mb-3"><IconVersions class="text-zinc-200" /><b class="text-zinc-200">Supports Minecraft Versions: </b> <span>{mcVersions.join(", ")}</span></p>
                    
                    <p class="text-zinc-400 flex items-center space-x-1 mb-3"><IconFileDescription class="text-zinc-200" /><b class="text-zinc-200">Changelog: </b> <span>{changelog}</span></p>

                    {#if modDeps.length > 0}
                    <p class="text-zinc-400 flex items-center space-x-1"><IconFilter class="text-zinc-200" /><b class="text-zinc-200">Modrinth Dependencies: </b> <span>{modDeps.map(obj => obj.title).join(', ')}</span></p>
                    {/if}
                    {#if smiDeps.length > 0}
                    <p class="text-zinc-400 flex items-center space-x-1 mb-3"><IconFilter class="text-zinc-200" /><b class="text-zinc-200">Smithed Dependencies: </b> <span>{smiDeps.map(obj => obj.name).join(', ')}</span></p>
                    {/if}

                    <p class="text-zinc-400 flex items-center space-x-1"><IconDeviceFloppy class="text-zinc-200"/><b class="text-zinc-200">Release Channel: </b> <span>{releaseChannel}</span></p>
                </div>
            ...
            <div class="mt-1 flex">
                ...
                <div class="w-1/3">
                    <button class="transition-all {authed > 0 ? "bg-orange-600" : "bg-orange-600/40 cursor-default"} {page == 3 ? "bg-green-700" : "" } rounded-md p-1 px-2 float-right text-lg font-semibold flex items-center space-x-1" on:click={() => {
                        if(page == 3) postFunction();
                        if(authed > 0) page++; 
                    }}>{#if page == 3}<IconCheck />{/if}<span>{page == 3 ? "Upload Version" : "Next"}</span></button>
                </div>
